...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Pre-marcar items ya visibles para evitar flash al agregar js-reveal-ready
    ...
    // Activar el sistema de reveal CSS (clase requerida por el selector)
    ...
    const populateModalByIndex = (index) => {
        ...
        if (modImg) {
            // Resetear estado antes de cargar nueva imagen
            modImg.style.display = 'none';
            if (modImgFallback) modImgFallback.style.display = 'none';

            if (!imgSrc) {
                // Sin fuente de imagen: mostrar fallback directamente
                if (modImgFallback) {
                    modImgFallback.style.display = 'flex';
                    if (modFallbackName) modFallbackName.textContent = 'Imagen no disponible';
                    if (modFallbackDesc) modFallbackDesc.textContent = `${year} — ${title}`;
                }
            } else if (rawImg && rawImg.naturalWidth === 0 && rawImg.complete) {
                // La imagen de la tarjeta ya falló: no reintentar, mostrar fallback
                if (modImgFallback) {
                    modImgFallback.style.display = 'flex';
                ...
                // Asignar src al final para que onload/onerror estén listos
                modImg.src = imgSrc;
            }
        }
    };
